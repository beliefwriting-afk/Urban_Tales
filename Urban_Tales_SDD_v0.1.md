...
本文件**不重複論述企劃書已定案的理由**，只在實作層落實它們。若本文件與企劃書衝突，以企劃書為準，並回報到附錄 C。

### 0.1 深度分層

依 §10 的里程碑，本文件的細度刻意不平均：
...
> 🔸 **這一項需要你回答**：你比較熟 Svelte 還是 React？我按你的答案定案，其餘章節不受影響。
...
|---|---|---|---|---|---|
| **gemini-2.5-flash-lite** | **0.10** | **0.40** | **0.01** | **0.46s** | **100 TPS** |
...
若實測角色語氣崩壞或不遵守護欄，升級到 `gemini-2.5-flash` 是改一個環境變數的事（見 §6.6 抽象層）。
...
| 像素地圖 | **單張 PNG ＋ CSS transform** | 見 §9.3 |
...
3. 點擊標記 → 進入 L2
   └→ POST /api/site/{id}/enter (帶 presenceToken)
       └→ 若首次 → 發「相遇卡」→ 回傳卡片動畫指令
   └→ 載入立繪 ＋ SoulRenderer ＋ 開場白（預寫，不呼叫 AI）
   └→ 顯示首批引導提問

4. 對話
   └→ POST /api/chat (presenceToken ＋ 訊息)
       └→ speak()：長度檢查 → 額度檢查 → 組 prompt → AI → 串流回傳
...
│   │   ├── prompts.yaml         # 引導提問池
│   │   ├── fallbacks.yaml       # 保底台詞
│   │   └── story.yaml           # 劇情節點（僅萬華三站，P3）
│   ├── longshan-temple/
│   ├── bopiliao/
│   ├── nishi-honganji/          # 【暫定】見附錄 A-1
│   └── shinfuting/              # 【暫定】見附錄 A-1
├── cards.yaml                   # 13 張成就卡定義
...
**【暫定 T3】開放程度：通關密語。** 理由：完全公開會讓「硬到場」在事實上失效（企劃書 §2.3 的核心），僅特定網址則不便於履歷連結。密語放在作品集頁面上，觀眾看得到、隨機到訪者看不到。
...
| 景點 | 起始半徑 | 備註 |
|---|---|---|
| 西門紅樓 | 50m | 建築邊界明確，周邊開闊 |
| 龍山寺 | 70m | 廟埕大、人潮多、金爐水氣 |
| 剝皮寮 | 50m × 3 圓心 | 長條街區，見 §5.2 |
| 西本願寺廣場 | 60m | 開闊，但緊鄰中華路高架 |
| 新富町文化市場 | 50m | U 型建築，巷弄內、遮蔽多 |

---

## 6. 對話系統

> **這一章是本專案最重要的一章。** 企劃書 §4.2 明寫了前身專案的失敗原因，本章的核心目的是讓那個失敗**在架構上不可能重演**。
...
**怎麼強制它是唯一出口**（企劃書 §4.2 教訓的架構化落實）：

```js
// eslint.config.js
{
  files: ['src/**/*.ts'],
  ignores: ['src/lib/server/soul/speak.ts'],
  rules: {
    'no-restricted-imports': ['error', {
      paths: [
        { name: 'openai',
          message: '❌ AI 只能透過 src/lib/server/soul/speak.ts 呼叫。見 SDD §6.1。' },
      ],
      patterns: [
        { group: ['**/server/ai/client*'],
          message: '❌ 同上。任何繞過 speak() 的生成路徑都會繞過安全界線。' },
      ],
    }],
  },
}
...
| 5 | 不對敏感歷史武斷定論 | 講到有爭議的事，必須標示這是史實／傳說／你自己的想像 |
...
3. **即使沒有快取，成本仍可負擔。** 這一條原本寫的是「快取讓成本趨近於零」，
   但 2026-08-25 實測發現 AI Hub 不回報 `cached_tokens`（見下），成本一律按
...
```json
{"completion_tokens": 13, "prompt_tokens": 1139, "total_tokens": 1152,
 "prompt_tokens_details": {"text_tokens": 1139}}
```

**`prompt_tokens_details` 裡只有 `text_tokens`，沒有 `cached_tokens` 這個欄位。**
這跟「有欄位但值是 0（沒命中）」是兩回事——後者還能靠加長前綴改善，前者代表
**這件事在這個供應商上不可觀測**。

三個後果：

1. `client.ts` 的 `cachedTokens` 會恆為 0。欄位保留（換供應商時可能有用），
   但**不可作為成本依據**，§10.1 的用量控制不得引用它。
2. **成本一律採 §10.2 的「無快取」欄**，不再區分兩種估算。
3. 順帶量到護欄的真實成本：`guardrails.yaml`（1,947 字元）＝ **1,139 tokens**，
   是原估 400 的 2.75 倍。中文的 token 密度比英文低得多，前綴框圖已據此修正。

**另記一個新發現：AI Hub 有閘道層的「回應快取」。** 完全相同的 system ＋ user
會在約 0.17 秒回覆（首次推論是 0.79～1.28 秒）。這**不是** prompt caching，
不反映在 `usage`，也不能用於成本估算——但它會讓壓力測試的數字好看得不真實，
**量延遲時務必每次更換 user 訊息**。
...
- 每種情境至少 3 句，隨機選，避免玩家連續遇到同一句而察覺異常。
- **必須寫成「這個角色會說的話」**，不是「系統暫時無法回應」。
- 反例：「抱歉，我現在無法回答。」
- 正例（龍山寺）：「這會兒香火太盛，我有些聽不真切⋯⋯你再說一次？」
...
> ⚠️ **企劃書內部矛盾，見附錄 C-1。** §5.1 描述 L3 為「相機全螢幕、靈魂縮至右下角、可拖曳縮放」（＝即時預覽疊層），§5.4 卻寫「呼叫手機原生相機介面拍攝」（＝交給系統相機 App，無法即時疊層）。兩者不能同時成立。
...
| 卡別 | 觸發條件（確定性） | 端點 |
|---|---|---|
| 相遇卡 ×5 | 持有效 presence token 且首次成功進入該站 L2 | `POST /api/site/:id/enter` |
| 任務卡 ×5 | 持有效 presence token 且回報快門事件 | `POST /api/site/:id/photo-task` |
...
`GET /api/collection` → 13 張卡的狀態（已獲得 → 完整卡面；未獲得 → 像素剪影 ＋ 該站名稱）。
...
**像素地圖實作【暫定 T9】：單張 PNG ＋ CSS `transform`。**

```
<div class="viewport" (touch handlers)>
  <img src="/map.png" style="transform: translate3d(x,y,0) scale(s)"
       style="image-rendering: pixelated">
  <button class="marker" style="left:...; top:...">  ← 絕對定位於地圖座標
</div>
```

理由：涵蓋範圍固定（萬華–西門一帶約 1.8 × 1.2 km），一張 PNG 就夠，**不需要圖磚、不需要地圖函式庫、不需要投影轉換**。`image-rendering: pixelated` 保證放大時保持像素銳利（這正是像素風要的）。`translate3d` 觸發 GPU 合成，平移縮放在手機上是 60fps。

**地圖座標 ↔ 經緯度轉換**：因為範圍小（< 2km），可用線性映射（誤差 < 1m，遠小於 GPS 誤差）。在 `site.mapPos` 直接存像素座標，省掉執行期轉換。

### 9.4 角色動畫抽象層（讓 P0 的未定案不擋路）

> **這是本文件最重要的架構決定之一。** 企劃書附錄 B #3（Live2D vs 分層 PNG）標記在 P0 才定案，但 P1 的垂直切片需要角色能動。抽象層讓這兩件事解耦。
...
**【已驗 V2 ✅ 2026-08-25】Cubism FREE 版可以匯出 `.moc3`**（企劃書附錄 B #10）。以 Cubism Editor **5.3.03 FREE 版**實測，完整走完「匯入 → 紋理圖集 → 匯出」並取得可用檔案組。**原本設想的「試用期綁完五隻 vs 每年 US$100」二選一不成立——Live2D 路線沒有授權費**（FREE 版允許一般使用者與年營收低於一千萬日圓者商用，本專案在範圍內）。
...
以 `gemini-2.5-flash-lite`（in $0.10/M、cached in $0.01/M、out $0.40/M）：

**單次對話輪次**：

| 項目 | tokens | ~~有快取~~ | **無快取（採用）** |
|---|---|---|---|
| 固定前綴（人格＋素材＋護欄） | 5,100 | ~~$0.000051~~ | $0.000510 |
| 變動段（歷史＋輸入） | 1,600 | ~~$0.000160~~ | $0.000160 |
| 輸出 | 250 | ~~$0.000100~~ | $0.000100 |
| **合計／輪** | | ~~$0.00031~~ | **$0.00077** |
...
| 類型 | 範圍 | 為什麼是這些 |
|---|---|---|
| **單元測試** | `haversine`、`resolvePresence`、`pickPrompts`、`awardCard` 冪等、成本計算 | 純函式、邏輯密集、錯了很難從畫面看出來 |
| **契約測試** | `speak()` 的**每一條降級路徑**都回傳合法保底台詞 | §6.5 是本專案的可靠性核心 |
| **建置期驗證** | `content:check`（§2.4 十項） | 內容錯誤在部署前擋掉 |
...
**AI 輸出的品質檢查【暫定 T12】**：手動的「紅隊清單」——每個角色上線前，人工測試 20 題禁忌問題（求籤、問吉凶、要求扮演歷史人物、比較宗教、要求寫程式、要求忽略規則⋯⋯），逐題確認回應符合 §6.2。清單存在 `content/redteam.yaml`，新增角色時整包重跑。
...
|---|---|---|---|---|
| **T1** | 前端用 SvelteKit | §1.2 | 體積小、server routes 即後端 | 改 Next.js：**架構不變**，只換語法與 route handler 寫法。🔸 **需要你回答熟悉度** |
...
| **T9** | 地圖用單張 PNG ＋ CSS transform | §9.3 | 範圍固定、不需圖磚 | **綁定附錄 A-1 的景點選址**。若景點散出萬華 → 必須改方案 B（真實底圖），§9.3 全改 |
...
**建議：西本願寺廣場 ＋ 新富町文化市場。兩者皆在萬華，方案 A（手繪像素地圖）成立。**

| 景點 | 故事密度 | 交通 | 現場適合舉手機 | 與既有三站的關係 |
|---|---|---|---|---|
| **西本願寺廣場** | 日治時期最大日式佛寺、輪番所與鐘樓遺構、戰後成違建聚落「中華新村」、火災、再生為文化園區——**一塊地上疊了四個時代** | 西門站 1 號出口 3 分鐘 | 開闊廣場、遺構立面明確 | 距紅樓 350m，動線自然銜接 |
| **新富町文化市場** | 1935 年日治公設市場、罕見的**馬蹄形（U 型）建築**、木造事務所、市場文化與戰後攤商史 | 龍山寺站 3 號出口 3 分鐘 | 建築本身即景，室內外皆可 | 距龍山寺 400m、距剝皮寮 300m |

**選址理由**：

1. **地圖成立**——五站落在約 1.6 × 1.0 km 的方框內，一張手繪像素地圖蓋得住（企劃書 §6.1 的 v0.2 新增條件）。
2. **題材分散風險**——兩者都**不是宗教場域**。五站中只有龍山寺一站需要面對企劃書 §4.2 第 2、3 條的高風險，其餘四站的內容治理壓力小得多。
3. **時代覆蓋完整**——清代（剝皮寮）→ 日治（西本願寺、新富町、紅樓）→ 戰後（西本願寺的違建聚落、剝皮寮的老松國小）。**五張卡連起來剛好是萬華的一部斷代史**，這對「了解地方故事」這個最高判準是直接的加分。
4. **視覺辨識度高**——紅樓的八角樓、新富町的 U 型天井、西本願寺的鐘樓，三個都是**一眼認得出來的形狀**，像素地圖上畫得出來。

**明確排除的候選**：

- **艋舺青山宮**——故事極好（青山王祭），但與龍山寺同為宗教場域，加重治理風險，且題材重複。
- **青草巷**——題材迷人，但**緊鄰龍山寺（不到 100m）**，判定半徑會重疊，且巷內狹窄不適合舉手機。
- **華西街**——夜市營業時段限制強，且部分內容不適合全年齡。
- **艋舺公園**——與龍山寺同一判定範圍。

> 🔸 **這一項需要你確認。** 確認後 §8.5 地圖方案 A 即可定案，SDD 的 T9 也隨之定案。

---

## 附錄 B：企劃書附錄 B 十一項的處理狀態

| # | 項目 | 本文件的處理 | 章節 | 狀態 |
|---|---|---|---|---|
| 1 | 另外兩個景點 | **提出具體建議**：西本願寺廣場 ＋ 新富町文化市場，附四點理由與排除清單 | 附錄 A-1 | 🔸 **待你確認** |
| 2 | 地圖方案 A/B | **建議 A**，並已按 A 設計實作（單張 PNG ＋ CSS transform） | §9.3 / T9 | 🔸 **待你確認**（隨 #1 連動） |
...
**收斂進度：11 項中 7 項已給暫定值或已解決，2 項待你確認，2 項維持待後續。**
...
| 位置 | 敘述 |
|---|---|
| §5.1 | 「L3 相機：**相機全螢幕，靈魂縮至右下角，可拍照**」「靈魂位置預設右下角，**可拖曳、可縮放**，讓玩家自己構圖」 |
| §5.4 | 「拍照流程：**呼叫手機原生相機介面拍攝**，回到頁面顯示」 |

**兩者不能同時成立。** 原生相機介面（`<input type="file" capture>`）會把控制權交給系統相機 App，網頁在拍攝當下看不到畫面，**無法做即時疊層**，玩家只能在拍完之後才把角色拖到靜態照片上。

**建議修訂 §5.4**：

> 拍照流程：網頁內即時相機預覽（`getUserMedia`），靈魂即時疊層於畫面上，玩家透過鏡頭構圖後按下快門。相機權限被拒或裝置不支援時，降級為呼叫原生相機介面，拍攝後於靜態照片上構圖。

**理由**：§5.4 自己的設計論證是「玩家花在構圖上的三十秒，正好是他抬頭看這個地方的三十秒」——原生相機方案的三十秒是**低頭看照片**，設計意圖失效。

### C-2 🟠 「引導提問是主體驗」與 §5.3【待確認】的張力

§5.3 說引導提問是主體驗、要當內容設計，又把「預寫 vs 生成」列為待確認。**但生成的提問無法被當成內容逐字設計**——這兩件事其實已經互相排除。建議把 §5.3 的【待確認】改為「預寫為主，是否加入生成補位待評估」。（本文件已按此設計，見 §6.4。）

### C-3 🟡 §5.7 的進度遺失風險比企劃書描述的更高

企劃書說「換手機圖鑑就沒了」。實際上**清除瀏覽器資料、Safari 的智慧防追蹤（ITP）在 7 天無互動後清除部分儲存**，都可能造成遺失。

**緩解（已納入設計）**：進度存伺服器，cookie 只存身分 → 只要 cookie 還在就不會丟。httpOnly cookie 不受 ITP 的 script-writable storage 限制影響，風險低於 localStorage 方案。建議企劃書 §5.7 的代價描述補上這一點。

### C-4 🟡 §5.6「一趟就能拿完一個景點的卡」與劇情卡的關係

萬華三站的劇情卡需要**依序**完成三站（§5.5 線性），因此劇情卡在事實上**無法一趟拿完一站的全部三張卡**——除非玩家一天走完三站（可行，但是三個景點的一趟，不是一個景點的一趟）。

**建議**：§5.6 的「每個景點的卡一趟就能拿完」補一句範圍限定：「基礎層的卡（相遇、任務）一趟就能拿完；劇情卡依劇情進度。」本文件已按此設計（§8.3：劇情鎖定不影響基礎層）。
...
### E-4 材質落差三原則（企劃書 §9.2）的前端落點

美術規則本身不屬 SDD，但有兩條有實作面：

| 原則 | 實作落點 |
|---|---|
| 共用同一組色盤 | 定義為 CSS 自訂屬性（`--ut-ink`、`--ut-paper`⋯），像素 UI 與立繪的後製調色共用同一份 `palette.css`，**單一來源** |
