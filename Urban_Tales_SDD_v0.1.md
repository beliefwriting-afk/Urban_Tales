...
```
┌── 可快取前綴（每站固定，不隨對話變動）────────  約 4,400 tokens
│  [1] 角色人格卡           ~800
...
3. **快取讓成本趨近於零。** 固定前綴的快取價是 $0.01/M，是原價的 1/10。

**【待驗 V1】Zeabur AI Hub 是否支援 prompt caching？** 官方文件未明載，模型頁有標示 cached token 價格。**驗證方式**：連發兩次相同前綴的請求，比對回應中 `usage.prompt_tokens_details.cached_tokens`。五分鐘可驗。若不支援，成本仍在可接受範圍（見 §10.2 保守估算）。
...
**【待驗 V2】Cubism FREE 版能否匯出 `.moc3`**（企劃書附錄 B #10，標記「開工前五分鐘」）。**在動手畫第一隻角色之前先驗。** 若不能，Live2D 路線的成本結構會從「試用期綁完五隻」變成「每年 US$100」，那是另一個決定。
...
| 項目 | tokens | 有快取 | 無快取 |
|---|---|---|---|
| 固定前綴（人格＋素材＋護欄） | 4,400 | $0.000044 | $0.000440 |
| 變動段（歷史＋輸入） | 1,600 | $0.000160 | $0.000160 |
| 輸出 | 250 | $0.000100 | $0.000100 |
| **合計／輪** | | **$0.00030** | **$0.00070** |

**推算**：

| 情境 | 有快取 | 無快取（保守） |
|---|---|---|
| 一站到訪（15 輪） | $0.005 | $0.011 |
| 一個玩家走完五站基礎層 | $0.023 | $0.053 |
| ＋ 三站劇情層（各 25 輪） | $0.045 | $0.105 |
| **1,000 名玩家全破** | **$45** | **$105** |
| 1,000 名玩家只玩一站 | $5 | $11 |

**結論**：這是**作品集規模下完全可負擔**的成本。全域日上限 $2 對應約 2,800–6,600 則訊息／日，遠超過作品集專案的實際流量。
...
| **P0-4** | （新增）Cubism FREE 匯出 moc3 | §9.4 待驗 V2 | **在 P0-1 之前做，五分鐘** |
| **P0-5** | （新增）AI Hub 連通與快取驗證 | §6.3 待驗 V1 | 打通 hnd1 端點、量測實際延遲、確認 cached_tokens |
...
| 8 | Zeabur AI Hub 模型與計費 | **已查證**：端點、OpenAI 相容性、點數制、模型清單與價格、延遲數據 | §1.2 / §10.2 | ✅ **已解決** |
| 9 | 全域 AI 用量上限機制 | **完整設計**：四層防線、事前檢查事後累加、觸發後與 AI 失效表現一致 | §10.1 | ✅ **已解決** |
| 10 | Cubism FREE 能否匯出 moc3 | **排進 P0-4，且明訂在畫第一隻角色之前做** | §9.4 / §15 | ⏸ 待驗（五分鐘） |
| 11 | 各階段時程 | 未處理——依企劃書，P0 結束後才估 | §15 | ⏸ 維持待確認 |

**收斂進度：11 項中 6 項已給暫定值或已解決，2 項待你確認，3 項維持待驗／待後續。**
