...
> 第八版，2026-09-04 更新（§15）。**新對話開始時，讀完 `CONTEXT.md` 之後接著讀這一份。**
...
**★★★ 最新一輪（2026-09-04）在 §15：內容規劃改版。** 兩個既有設計被推翻：
素材庫從「13 條 items ＋ kind／topic」改成 **一段 `facts` ＋ 幾條 `legends`**；
引導提問從「≥12 題 ＋ tier 挑選」改成 **恰好 3 題、寫死**。
schema、content:check、範本、三站素材、SDD、企劃書、CONTEXT **全部一起改完**。
⚠️ 三站的 `materials.yaml` 現在是**機械轉檔的中間狀態**，等君和重寫（§15.4）。
...
## 15. 內容規劃改版（2026-09-04）

> 君和開始規劃各景點的對話內容，過程中定了新的內容形狀，**推翻了 SDD 與企劃書
> 各一項既有設計**。這一節記錄改了什麼、為什麼、以及連帶動到哪些檔案。
...
### 15.3 改了哪些檔案（全部一起改，避免又留下不一致）

| 檔案 | 改了什麼 |
|---|---|
| `content/schema.ts` | 兩個 schema 重寫，`MaterialKind` 移除 |
| `scripts/content-check.ts` | #3 改判準、**#4 改寫**、#5 從 ≥12 改成恰好 3 ＋ 15 字警告 |
| `content/_template/materials.yaml`、`prompts.yaml` | 範本整份重寫 |
| `content/sites/*/materials.yaml`（三站） | **機械轉檔**，見 §15.4 |
| `src/lib/server/soul/speak.ts` | `SpeakResult.nextPrompts` 移除 |
| `Urban_Tales_SDD_v0.1.md` | §2.3 schema、§2.4 十項表、**§6.4 的 `pickPrompts()` 整段作廢**、附錄 A T5、附錄 B #5、附錄 E 第 5 條 |
| `Urban_Tales_企劃書_v0.2.md` | §2 基礎層、§4.2 第 5 條、§4.3、**§5.3 整段重寫**、附錄 B #5 結案 |
| `CONTEXT.md` | 內容治理硬規則第 5 條、文件地圖、目前狀態 |

★ **`content:check` 的規則編號沒有重用。** 舊的 #4（prompts 的 topic 要在 materials
找得到）失去依據之後，新的 #4 是另一條規則（legends 空的要唸出來），但**編號的歷史
在 SDD §2.4 留著刪除線**——重用編號會讓「#4 是什麼」在兩份文件裡有兩個答案。

### 15.4 ⚠️ 三站的 `materials.yaml` 是「機械轉檔的中間狀態，還不能用」

轉檔腳本做的事：

- 原本標 `已知史實` 的 12 條，**照原本的 topic 分段、原順序、一字不改**串進 `facts`，
  段落前留著 `【原本的 topic】` 當標記
- 原本標 `民間傳說` 的搬進 `legends`（紅樓 1 條、剝皮寮 1 條、龍山寺 0 條）
- 原本標 `角色想像` 的**移出素材庫**，原文保留在檔案最下方的註解裡
  （龍山寺有 1 條：「人們把心事帶到這裡⋯⋯」）

**君和要做的事：把 `facts` 重寫成一段連貫的敘述**，並把 `【topic】` 標記刪掉。
現在它是碎片的拼接——那正是改版要解決的問題。

⚠️ `sources` 是把原本每一條的 `source` **去重收集**起來的（各 10–11 個）。
重寫 facts 之後要回頭確認每一個都還對得上。

⚠️ 三站都是 `status: draft`，所以 `content:check` **目前只驗 site.yaml**，
上面那些檔案的問題不會讓建置失敗。轉 `playable` 那一刻才會全部生效。

### 15.5 🔜 這一輪之後還沒做的

- **君和寫內容**：三站的 `facts` 重寫、`legends` 補齊、`prompts.yaml` 各 3 題、
  `fallbacks.yaml` 四種情境。原料在 `City_Soul_AI_對話素材盤點.md`。
- **前端還沒有引導提問元件**。`ChatLayer.svelte` 目前沒有那三個按鈕，
  mock 層也沒有假資料——切片 7 要補。
- **靈魂名稱與性格君和會自己改**（`soul.yaml` 的 `name` 與 `persona`）。
  那部分不動 schema。
- **萬華劇情另外規劃**（P3），這一輪只處理基礎層。
