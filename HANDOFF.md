...
> §13.6、§14.6、§15.4（內容已經不成立）。編號留空——重用編號會讓「§13 是什麼」
...
| 內容 | ✅ 六站各四份 YAML 就位，**全部 `status: draft`**（§16）。缺 `fallbacks.yaml` |
| 角色美術 | ⬜ 一張立繪都還沒有（P0-1） |
...
1. **`fallbacks.yaml`：五種情境 × 六站。** 只有君和能寫（措辭即成品，要逐字審）。
   ★ **霞海城隍廟的姻緣類轉向台詞優先**——那是全六站最容易踩到護欄第 3 條的地方（§16.5）。
   **這是切片 5 的前提，沒有它 chat 端點沒東西可回退。**
2. **切片 5** `POST /api/chat` ＋ `speak()` 九步 —— 最大的一塊。
3. **切片 6** 快門回報 ＋ 圖鑑（`GET /api/collection`，`cards.ts` 的 `listCards()` 已備好）。
4. **切片 7** 前端 `mock/session.svelte.ts` 換成真 API。
   ⚠️ **不只是換資料來源**，有三個落差要處理，見 §14.8。最重要的是
   **前端目前有能力自己宣告「我到了」**，那段程式碼必須消失，不是改成讀 API。
5. **P0-5 重做** —— 換 Gemini 之後延遲與成本都要重量，**連 thinking level 一起量**（§13.7）。
6. **P0-1** 角色動畫（**6 隻**）、**P0-2** 實地量測（**6 組座標**，大同區三組完全沒量過）。
7. **P0-3** 相機 ＋ 角色合成、**地圖色調**（§12.6）。

### 第一站要轉 `playable` 時會依序撞到

1. `content:check #2` —— 缺 `fallbacks.yaml`
2. `content:check #7b` —— `cards.yaml` 沒有那一站的相遇卡（**這條比 #7 早撞到**）
3. `content:check #8` —— `soul.yaml` 的 `art` 還是 null（等 P0-1）
4. 三關都過之後，`smoke:api` 的「進入景點」才測得到成功路徑——**記得回去把那幾條補上**
5. 六站**全部** playable 之後，`content:check #7` 才開始強制卡片總數 15（6/6/3）
...
| 卡片 id 是 `encounter-${s.id}` 拼出來的 | 由伺服器查 `cards.yaml` 給 | 正是 §14.3 拍板 #3 否決的那個約定。前端要改成用伺服器回的 id |
