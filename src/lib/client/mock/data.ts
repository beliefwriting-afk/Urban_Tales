 ...
 *
 * ★ **mock 停在萬華三站是刻意的，規格是六站。**
 *   所以下面 CARDS 生成出來是 9 張（3 相遇 ＋ 3 任務 ＋ 3 劇情），
 *   而規格是 **15 張（6 相遇 ＋ 6 任務 ＋ 3 劇情）**，定義在 content/（六站都在了）。
 *   ⚠️ 這一層是切片 7 要整個換掉的，屆時站數與卡片會一起對齊真後端。
 *   四個落差見 HANDOFF §14.8。
 *
 * 給 Python 背景的對照：
...
export type Site = {
	...
	sensingM: number;
	/** 召喚半徑（公尺）——進來才點得動。對應 SDD §5.2 的到場判定演算法 */
	summonM: number;
};
