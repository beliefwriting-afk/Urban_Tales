 ...
 *
 * ⚠️ 2026-08-28：`drawTerrain` 已經**不再被使用**。
 *   地圖圖層換成了 static/map/ 的真實 OpenStreetMap 圖磚（見 tiles.ts 與 MapLayer.svelte）。
 *   這個函式先留著當退路，等圖磚在實機上驗過就可以整段刪掉。
 *   `drawAvatar` 與 AVATAR_* 仍然在用，不要一起刪。
 *
 * 當初為什麼是程序生成：
 *   地圖方案還沒拍板時，用程序生成的佔位地形可以先把介面、互動、
 *   像素質感全部確認完，之後換成真圖磚時只換這個檔，其餘一行不動。
 *   結果證明這個賭注是對的——換圖磚時 UI 元件確實一行都沒改。
 *
 * 像素感從哪裡來：
