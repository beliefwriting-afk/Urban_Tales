 ...
 *
 * 【暫定 T3】開放程度：通關密語。完全公開等於沒有硬到場。
 * 密語寫在作品集頁面，觀眾看得到、隨機到訪者看不到。
 *
 * ⚠️ 「密語」與「特定網址」是**同一個機制**——這一支就是 `GET /demo?key=`，
 *   本身即一個可放在履歷上的網址。真正的替代方案是綁定身分（登入 ＋ 白名單），
 *   代價是履歷的觀眾進不來。見企劃書 §5.8 的三個層級。
 *
 * ⚠️ 密語應為**隨機字串**。下面的常數時間比較擋的是 timing attack，
 *   擋不了字典猜測，而這一支沒有速率限制。反正密語是複製貼上的，
 *   沒有人需要記住它。
 *
 * ★ 為什麼是第三個檔案、第三把金鑰
